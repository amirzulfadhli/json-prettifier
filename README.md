# JSON Prettifier

A lightweight browser-based JSON utility for formatting, minifying, and copying JSON directly in the browser.

## Features
- Format JSON with readable indentation
- Minify JSON into a compact single-line string
- Copy the generated output to the clipboard
- Clear the input and output panels instantly
- Keyboard shortcuts for faster editing
- Accessible status messages and validation feedback

## Usage
1. Paste a JSON payload into the input area.
2. Click Format or Minify, or use the keyboard shortcuts:
   - Ctrl/Cmd + Enter: Format
   - Ctrl/Cmd + Shift + M: Minify
3. Review the output and use Copy output when needed.

## Notes
- The app runs entirely on the client side and does not send your JSON to a server.
- Invalid JSON produces a clear error message with the parsing issue.
